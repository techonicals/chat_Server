/* to compile me in Linux, type:  gcc -o server t1.c -lpthread */  
 ...
 int main(int argc, char *argv[]) {  
  ...
  if( argc == 2 )       
      ...
      server_addr.sin_port=htons(portnum);  
       printf("IP ADDRESS:t%sn",inet_ntoa(server_addr.sin_addr));  
      /*=creating socket=*/  
   ...
   if(sockfd == -1){  
      ...
      printf("socketttcreated.n");  
   if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes,sizeof(int)) == -1) {  
      printf("setsockopt error");     // debugging  
      exit(1);  
   }else printf("reusingttportn");  
      /*=binding socket=*/  
   if(bind(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr))==-1){  
           printf("binding failedn");     // debugging  
      exit(1);}  
   else   
      printf("bindingttsuccess.nn");  
 printf("ttPRESS CTRL+z TO VIEW ONLINE CLIENTSnn");  
      /*=socket on listening mode=*/  
      listen(sockfd, BACKLOG);  
      printf("waiting for clients......n");  
   ...
   while(1){  
           ...
           if(recv(new_fd,username,sizeof(username),0)>0);  
             ...
             printf("t%d->%s JOINED chatroomn",new_fd,username);  
             sprintf(buffer,"%s IS ONLINEn",username);  
           ...
           send(sf2,buffer ,sizeof(buffer),0);  
              ...
              printf("server got connection from %s & %dnn",inet_ntoa(client_addr.sin_addr),new_fd); // debugging  
 ...
 void *server(void * arguments){  
  ...
  addr      a;  
       ...
       a =h ;  
           do{  
           a = a->next;  
           sprintf( ubuf," %s is onlinen",a->username );  
   ...
   while(1){  
      ...
      jmp:     printf("%d ->%s left chat deleting from listn",ts_fd,uname);  
           sprintf(buffer,"%s has left the chatn",uname);  
           addr a = h ;  
           do{  
                a = a->next;  
                sfd = a->port;  
 ...
 void Display( const head h )  
      {  
        addr a =h ;  
        if( h->next == NULL )  
      printf( "NO ONLINE CLIENTSn" );  
        else  
        {  
           do  
           {  
             a = a->next;  
             printf( "%d->%s t", a->port,a->username );  
           } while( a->next != NULL );  
           printf( "n" );  
        }  
      }  
    ...
    if( h->next == NULL ) {  
     printf( "......BYE.....nno clients nn" );  
      exit(0);  
       ...
       printf("%d clients closednn",i);         
       }  
  }  
 void zzz(){  
   printf("rDISPLAYING ONLINE CLIENTSnn");  
   Display(h);  
  }  
 /*=====================================================================================================*/  